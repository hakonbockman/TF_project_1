...
def draw_learning_curves(history, model_title):
    ...
    plt.plot(history['val_accuracy'], color='r')
    plt.show()

    plt.plot(history['mean_square_error'], color='b')
    plt.plot(history['val_mean_squared_error'], color='r')
    plt.show()
    '''
...
if __name__ == '__main__':
    ...
    for i, (k, v) in enumerate(return_image_path_dict().items()):
        ...
        path = v
        #model_title = str(path.split("/")[-2]) #path must end with /
